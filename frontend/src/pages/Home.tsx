...
import { BookOpen, Plus, ChevronRight, Settings } from 'lucide-react';

export default function Home() {
  const navigate = useNavigate();
  const { subjects, activeSubject, create, setActive } = useSubjectStore();
            ...
            subjects.map((subject) => {
              ...
              return (
                <div key={subject.id}
                  ...
                  className={'flex items-center justify-between p-4 rounded-xl border cursor-pointer transition-all ' + (isActive ? 'bg-brand-50 border-brand-200' : 'bg-white border-gray-100 hover:border-gray-200 hover:shadow-sm')}>
                  <div className="flex items-center gap-3">
