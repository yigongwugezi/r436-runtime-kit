...
import { deleteResource, getResourceById, regenerateResource, updateStudyStatus, autoAdvanceNode, getResourceKnowledgeGraph, batchUpdateStudyStatus, batchSetBookmark, batchExportResources } from '../api/resources';
...
const qualityLabel: Record<string, string> = { passed: '质检通过', repaired: '已修复', fallback: '本地兜底', failed: '需复核', needs_review: '需复核', fallback_passed: '兜底通过', blocked: '已屏蔽', provider_unavailable: '服务不可用' };
const qualityBadge: Record<string, string> = { passed: 'bg-success-50 text-success-700', repaired: 'bg-blue-50 text-blue-700', fallback: 'bg-warning-50 text-warning-700', failed: 'bg-error-50 text-error-700', needs_review: 'bg-error-50 text-error-700', fallback_passed: 'bg-warning-50 text-warning-700', blocked: 'bg-error-50 text-error-700', provider_unavailable: 'bg-gray-50 text-gray-600' };
...
function ResourceListView({
  ...
  r.type === 'video' ? (
    ...
    <button onClick={async (e) => { e.stopPropagation(); try { const res = await fetch(r.content); if (!res.ok) throw new Error(); const blob = await res.blob(); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `${r.title || '演示文稿'}.pptx`; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); } catch { window.open(r.content, '_blank'); } }} className="px-3 py-1.5 rounded-lg bg-orange-50 text-orange-600 font-medium hover:bg-orange-100 transition-colors">📥 下载</button>
  )
) : <div className="flex items-center gap-1"><span className={`px-2 py-0.5 rounded ${diffBadge[r.difficulty]}`}>{diffLabel[r.difficulty]}</span>{r.qualityStatus && <span className={`px-2 py-0.5 rounded ${qualityBadge[r.qualityStatus] || 'bg-surface-100 text-surface-500'}`}>{qualityLabel[r.qualityStatus] || r.qualityStatus}</span>}</div>}</div></div>
                    </div>
                  ); })}
                </div>
              ) : (
