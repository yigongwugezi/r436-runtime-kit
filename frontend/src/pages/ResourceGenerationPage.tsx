...
export default function ResourceGenerationPage() {
  ...
  const updateUrl = useCallback((nextPrompt: string, nextTypes: GeneralResourceType[], nextTemplate?: string) => {
    const next = new URLSearchParams();
    if (nextPrompt) next.set('q', nextPrompt);
    if (nextTypes.length) next.set('types', nextTypes.join(','));
    const tpl = nextTemplate ?? selectedTemplate;
    if (tpl) next.set('tpl', tpl);
    setSearchParams(next, { replace: true });
  }, [setSearchParams, selectedTemplate]);
