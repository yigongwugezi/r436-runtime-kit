...
export default function ProfilePage() {
  ...
  return <div className="space-y-6 pb-8">
    <header className="rounded-2xl bg-gradient-to-r from-blue-600 to-violet-600 p-6 text-white">
      ...
      <div className="mt-4 grid grid-cols-2 gap-3 text-sm md:grid-cols-4"><span>目标：{subject.learning_goal || '待补充'}</span><span>每日：{subject.daily_minutes ? `${subject.daily_minutes} 分钟` : '待补充'}</span><span>学习周期：{subject.deadline || '待补充'}</span><span>信息完整度：{Math.round((profileV2.profile_completeness || 0) * 100)}%</span></div>
    </header>
    ...
    <section className="rounded-2xl bg-white p-5 shadow-sm"><div className="mb-4 flex items-center justify-between"><h3 className="font-semibold">学习情境与偏好</h3><button onClick={() => setEditing(!editing)} className="inline-flex items-center gap-1 text-sm text-blue-600"><Edit3 size={14} />{editing ? '取消' : '编辑'}</button></div>
      ...
      </div> : <div className="grid gap-3 text-sm md:grid-cols-3">{[["学习目标", subject.learning_goal], ["时间安排", subject.daily_minutes ? `每天${subject.daily_minutes}分钟` : '待补充'], ["专业背景", subject.background], ["已有经验", (subject.prior_experience || []).join('、')], ["内容偏好", preferenceText(subject.content_preferences || [])], ["资源偏好", preferenceText(subject.resource_preferences || [])]].map(([label, value]) => <div key={label} className="rounded-xl bg-surface-50 p-3"><p className="text-xs text-surface-400">{label}</p><p className="mt-1 text-surface-700">{value || '待补充'}</p></div>)}</div>}
    </section>
