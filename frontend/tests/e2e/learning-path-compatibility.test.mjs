...
class Cdp {
  constructor(socket) {
    this.socket = socket; this.nextId = 1; this.pending = new Map(); this.exceptions = [];
    socket.addEventListener('message', ({ data }) => {
      const message = JSON.parse(data);
      if (message.id) return this.pending.get(message.id)?.(message);
  ...
  async evaluate(expression) { const result = await this.call('Runtime.evaluate', { expression, returnByValue: true, awaitPromise: true }); if (result.exceptionDetails) throw new Error(result.exceptionDetails.exception?.description || result.exceptionDetails.text || 'browser evaluation failed'); return result.result.value; }
}
...
test('real Edge keeps legacy learning-path fields, mode, and section context compatible', { timeout: 90000 }, async () => {
  const tempDir = await mkdtemp(join(tmpdir(), 'eduagent-path-e2e-'));
  let backend; let frontend; let edge; let socket;
  try {
    backend = start('python', ['backend/tests/learning_path_browser_fake_server.py'], { cwd: repoDir, env: { ...process.env, PYTHONPATH: 'backend', EDUAGENT_SKIP_ENV_FILE: '1', LLM_PROVIDER: 'mock', DATABASE_URL: `sqlite:///${join(tempDir, 'browser.db')}`, PORT: String(backendPort) } });
    await waitFor(`http://127.0.0.1:${backendPort}/api/health`, 'isolated backend');
    const fixtureResponse = await fetch(`http://127.0.0.1:${backendPort}/api/learning-path?sessionId=path-session&subjectId=fixture-subject`);
    assert.equal((await fixtureResponse.json()).data.path.id, 'path-legacy');
    const registration = await fetch(`http://127.0.0.1:${backendPort}/api/auth/register`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ phone: '13900000012', password: 'e2e-only', nickname: 'path-fixture', role: 'student' }) });
    assert.equal(registration.ok, true);
    const identity = await registration.json();
    const subjectResponse = await fetch(`http://127.0.0.1:${backendPort}/api/subjects`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${identity.access_token}` }, body: JSON.stringify({ name: '路径测试科目' }) });
    assert.equal(subjectResponse.ok, true);
    const subjectBody = await subjectResponse.json();
    const subject = subjectBody.data?.subject || subjectBody.subject;
    ...
    await setText(cdp, "input[type='text']", '13900000012');
    await setText(cdp, "input[type='password']", 'e2e-only');
    await cdp.evaluate("document.querySelector('form').requestSubmit()");
    try { await waitForBrowser(cdp, "location.pathname === '/'", 'authenticated home'); }
    catch { throw new Error(`login did not reach home at ${await cdp.evaluate('location.href')} (${(await cdp.evaluate('document.body.innerText')).slice(0, 240)})`); }
    await cdp.evaluate(`localStorage.setItem(${JSON.stringify(`r436_runtime_active_subject_${identity.learner.id}`)}, ${JSON.stringify(JSON.stringify(subject))}); localStorage.setItem(${JSON.stringify(`r436_runtime_subjects_${identity.learner.id}`)}, ${JSON.stringify(JSON.stringify([subject]))}); localStorage.setItem(${JSON.stringify(`r436_runtime_session_${identity.learner.id}_${subject.id}`)}, 'path-session'); true`);
    await cdp.call('Page.reload');
    await waitForBrowser(cdp, "location.pathname === '/'", 'reloaded authenticated home');
