...
def resolve_current_learning_path(
    ...
    """Resolve a scope-safe current path; unscoped legacy rows are never guessed."""
    if path_id:
        path = db.get(LearningPathModel, path_id)
        if path is None or path.session_id != session_id:
            return None
        session = db.get(SessionModel, session_id)
        if session is None or (session.learner_id and learner_id and session.learner_id != learner_id):
            return None
        if path.subject_id and path.subject_id != subject_id:
            return None
        return path
    pointer = db.query(CurrentLearningPathModel).filter_by(
        learner_id=learner_id, session_id=session_id, subject_id=subject_id,
    ...
    # An unscoped legacy path must be selected explicitly; never guess it.
