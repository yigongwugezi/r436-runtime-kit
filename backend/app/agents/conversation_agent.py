...
"""
                ...
                for attempt in range(3):
                    try:
                        ...
                        if thinking:
                            ...
                            logger.warning("DeepThink: no <thinking> tag found in LLM response (first 200 chars: %s)", raw_response[:200])
                        if suggestions:
    ...
    def _extract_reply_and_facts(raw: str) -> tuple[str, dict, str, str, str, list]:
        ...
        # 提取 suggestions（推荐操作按钮）
        sug_match = re.search(r'<suggestions>(.*?)</suggestions>', text, re.DOTALL)
        if sug_match:
            sug_text = sug_match.group(1).strip()
            text = re.sub(r'<suggestions>.*?</suggestions>', '', text, flags=re.DOTALL)
            # 解析每行 JSON: {"label": "...", "prompt": "..."}
            for line in sug_text.splitlines():
                line = line.strip()
                if line.startswith('{') and line.endswith('}'):
                    try:
                        obj = json.loads(line)
                        if obj.get("label") and obj.get("prompt"):
                            suggestions.append({"label": str(obj["label"])[:10], "prompt": str(obj["prompt"])})
                    except json.JSONDecodeError:
                        pass
            suggestions = suggestions[:3]  # 最多3个
...
action："""
    ...
    def _call_llm(self, messages):
        ...
        search_enabled = getattr(self, '_search_enabled', False)
        deep_think_enabled = getattr(self, '_deep_think_enabled', False)
        return self.llm_client.chat(messages=messages, temperature=0.7, search=search_enabled, max_tokens=800)
