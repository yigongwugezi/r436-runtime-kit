...
def test_agents_generate_from_course_knowledge_base() -> None:
    ...
    assert_true("time_budget" in planner_metadata["priority_basis"], "planner metadata should expose time-budget priority")
    assert_true("time_budget_tight" in planner_metadata["risk_flags"], "planner metadata should expose tight time risk")
    assert_true(planner_metadata["estimated_days"] == 2, "planner metadata should expose estimated days")
